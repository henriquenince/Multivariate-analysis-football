...
n_fatores <- 6 # testei com 5, 6 e 7 fatores (6 foi o melhor)
...
colnames(scores_fatoriais) <- c("Score_MR1", "Score_MR4", "Score_MR2", "Score_MR5", "Score_MR3", "Score_MR6")
...
scores_fatoriais <- scores_fatoriais %>%
  mutate(Score_MR1 = normalize_0_100(scores_fatoriais$Score_MR1),
         ...
         Score_MR5 = normalize_0_100(scores_fatoriais$Score_MR5),
         Score_MR3 = normalize_0_100(scores_fatoriais$Score_MR3),
         Score_MR6 = normalize_0_100(scores_fatoriais$Score_MR6))

# Juntando bancos

banco_volantes <- cbind(banco_volantes[, 1:7], as.data.frame(scores_fatoriais))

# Calculando score geral

  ## Pesos pela variancia explicada

pesos <- c(MR1 = 0.338, MR4 = 0.148, MR2 = 0.138, MR5 = 0.083, MR3 = 0.081, MR6 = 0.057)
pesos <- pesos / sum(pesos)  # normalizar para somar 1

banco_volantes <- banco_volantes %>%
...
summary(banco_volantes$Score_Geral_Var)
...
banco_volantes <- banco_volantes %>% mutate(Score_Final = Score_Geral_Var)

banco_volantes <- banco_volantes[, c(1, 3, 4, 8, 9, 10, 11, 12, 13, 15)]

write_xlsx(banco_volantes, "scores_volantes.xlsx")
