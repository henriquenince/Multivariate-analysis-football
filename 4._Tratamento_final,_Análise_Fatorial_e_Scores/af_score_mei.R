...
n_fatores <- 2 # testei com 2, 3 e 4 fatores (2 foi o melhor)
...
colnames(scores_fatoriais) <- c("Score_MR1", "Score_MR2")
...
banco_meias <- cbind(banco_meias[, 1:7], as.data.frame(scores_fatoriais))

# Calculando score geral

  ## Pesos pela variancia explicada

pesos <- c(MR1 = 0.481, MR2 = 0.310)
pesos <- pesos / sum(pesos)  # normalizar para somar 1

banco_meias <- banco_meias %>%
  mutate(Score_Geral_Var = (Score_MR1 * pesos["MR1"]) +
           (Score_MR2 * pesos["MR2"]))

summary(banco_meias$Score_Geral_Var)
...
banco_meias <- banco_meias %>% mutate(Score_Final = Score_Geral_Var)

banco_meias <- banco_meias[, c(1, 3, 4, 8, 9, 11)]

write_xlsx(banco_meias, "scores_meias.xlsx")
