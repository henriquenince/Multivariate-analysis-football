...
colnames(scores_fatoriais) <- c("Score_MR1", "Score_MR3", "Score_MR4", "Score_MR2")
...
banco_pontas <- cbind(banco_pontas[, 1:7], as.data.frame(scores_fatoriais))

# Calculando score geral

  ## Pesos pela variancia explicada

pesos <- c(MR1 = 0.350, MR3 = 0.194, MR4 = 0.164, MR2 = 0.136)
pesos <- pesos / sum(pesos)  # normalizar para somar 1

banco_pontas <- banco_pontas %>%
  mutate(Score_Geral_Var = (Score_MR1 * pesos["MR1"]) +
           (Score_MR3 * pesos["MR3"]) +
           (Score_MR4 * pesos["MR4"]) +
           (Score_MR2 * pesos["MR2"]))

summary(banco_pontas$Score_Geral_Var)
...
banco_pontas <- banco_pontas %>% mutate(Score_Final = Score_Geral_Var)

banco_pontas <- banco_pontas[, c(1, 3, 4, 8, 9, 10, 11, 13)]

write_xlsx(banco_pontas, "scores_pontas.xlsx")
