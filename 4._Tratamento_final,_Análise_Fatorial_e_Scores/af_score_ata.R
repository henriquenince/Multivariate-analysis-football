...
n_fatores <- 4 # testei com 4, 5 e 6 fatores (4 foi o melhor)
...
colnames(scores_fatoriais) <- c("Score_MR1", "Score_MR3", "Score_MR2", "Score_MR4")
...
banco_atacantes <- cbind(banco_atacantes[, 1:7], as.data.frame(scores_fatoriais))

# Calculando score geral

  ## Pesos pela variancia explicada

pesos <- c(MR1 = 0.410, MR3 = 0.182, MR2 = 0.111, MR4 = 0.099)
pesos <- pesos / sum(pesos)  # normalizar para somar 1

banco_atacantes <- banco_atacantes %>%
  mutate(Score_Geral_Var = (Score_MR1 * pesos["MR1"]) +
           (Score_MR3 * pesos["MR3"]) +
           (Score_MR2 * pesos["MR2"]) +
           (Score_MR4 * pesos["MR4"]))

summary(banco_atacantes$Score_Geral_Var)
...
banco_atacantes <- banco_atacantes %>% mutate(Score_Final = Score_Geral_Var)

banco_atacantes <- banco_atacantes[, c(1, 3, 4, 8, 9, 10, 11, 13)]

write_xlsx(banco_atacantes, "scores_atacantes.xlsx")
