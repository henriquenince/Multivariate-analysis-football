...
scores_lat <- read_excel("scores_laterais.xlsx", sheet = 1)
...
fviz_nbclust(scores_lat["Score_Final"], kmeans, method = "wss", k.max = 9, nstart = 25) +
...
fviz_nbclust(scores_lat["Score_Final"], kmeans, method = "silhouette", k.max = 9, nstart = 25) +
...
nb_result <- NbClust(data = scores_lat["Score_Final"], distance = "euclidean", 
...
kmeans_result <- kmeans(scores_lat["Score_Final"], centers = k_ideal, nstart = 25)
scores_lat$Cluster <- as.factor(kmeans_result$cluster)

# Criando tabela de medias por cluster

cluster_media <- scores_lat %>% group_by(Cluster) %>%
...
scores_lat <- scores_lat %>% left_join(cluster_media, by = "Cluster") %>%
...
silhouette_result <- silhouette(as.numeric(scores_lat$Cluster), dist(scores_lat["Score_Final"]))
...
# 
# scores_lat %>% group_by(Cluster) %>% summarise(p_valor_shapiro = shapiro.test(Score_Final)$p.value) 

  ## Kruskal-Wallis

kruskal_result <- kruskal.test(Score_Final ~ Cluster, data = scores_lat)
kruskal_result
...
ggplot(scores_lat, aes(x = Cluster, y = Score_Final, fill = Cluster)) +
