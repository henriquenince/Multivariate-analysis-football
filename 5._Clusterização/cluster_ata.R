...
fviz_nbclust(scores_ata["Score_Final"], kmeans, method = "wss", k.max = 12, nstart = 25) +
...
fviz_nbclust(scores_ata["Score_Final"], kmeans, method = "silhouette", k.max = 12, nstart = 25) +
...
nb_result <- NbClust(data = scores_ata["Score_Final"], distance = "euclidean", 
...
kmeans_result <- kmeans(scores_ata["Score_Final"], centers = k_ideal, nstart = 25)
scores_ata$Cluster <- as.factor(kmeans_result$cluster)

# Criando tabela de medias por cluster

cluster_media <- scores_ata %>% group_by(Cluster) %>%
...
scores_ata <- scores_ata %>% left_join(cluster_media, by = "Cluster") %>%
...
silhouette_result <- silhouette(as.numeric(scores_ata$Cluster), dist(scores_ata["Score_Final"]))
...
# 
# scores_ata %>% group_by(Cluster) %>% summarise(p_valor_shapiro = shapiro.test(Score_Final)$p.value) 

  ## Kruskal-Wallis

kruskal_result <- kruskal.test(Score_Final ~ Cluster, data = scores_ata)
kruskal_result

  ## Dunn

dunn_result <- dunnTest(Score_Final ~ Cluster, data = scores_ata, method = "bonferroni") 
dunn_result
...
ggplot(scores_ata, aes(x = Cluster, y = Score_Final, fill = Cluster)) +
