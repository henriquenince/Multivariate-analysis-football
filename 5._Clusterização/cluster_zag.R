...
scores_zag <- read_excel("scores_zagueiros.xlsx", sheet = 1)
...
fviz_nbclust(scores_zag["Score_Final"], kmeans, method = "wss", k.max = 9, nstart = 25) +
...
fviz_nbclust(scores_zag["Score_Final"], kmeans, method = "silhouette", k.max = 9, nstart = 25) +
...
nb_result <- NbClust(data = scores_zag["Score_Final"], distance = "euclidean", 
...
kmeans_result <- kmeans(scores_zag["Score_Final"], centers = k_ideal, nstart = 25)
scores_zag$Cluster <- as.factor(kmeans_result$cluster)

# Criando tabela de medias por cluster

cluster_media <- scores_zag %>% group_by(Cluster) %>%
...
scores_zag <- scores_zag %>% left_join(cluster_media, by = "Cluster") %>%
...
silhouette_result <- silhouette(as.numeric(scores_zag$Cluster), dist(scores_zag["Score_Final"]))
...
# 
# scores_zag %>% group_by(Cluster) %>% summarise(p_valor_shapiro = shapiro.test(Score_Final)$p.value) 

  ## Kruskal-Wallis

kruskal_result <- kruskal.test(Score_Final ~ Cluster, data = scores_zag)
kruskal_result

  ## Dunn

dunn_result <- dunnTest(Score_Final ~ Cluster, data = scores_zag, method = "bonferroni")
dunn_result
...
ggplot(scores_zag, aes(x = Cluster, y = Score_Final, fill = Cluster)) +
